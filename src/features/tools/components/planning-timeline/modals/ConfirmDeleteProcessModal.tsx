...
import { AccountTree } from "@nine-thirty-five/material-symbols-react/outlined";

interface ConfirmDeleteProcessModalProps {
  opened: boolean;
  onClose: () => void;
  onConfirm: () => void;
}

export default function ConfirmDeleteProcessModal({
  opened,
  onClose,
  onConfirm,
}: ConfirmDeleteProcessModalProps) {
  return (
    <Modal
      opened={opened}
      onClose={onClose}
    ...
    >
      <Box style={{ padding: "1rem 0.5rem 0.5rem", textAlign: "center" }}>
        ...
        >
          <AccountTree width="100" height="100" />
        </Box>
        <Text
          fw={500}
          size="sm"
        ...
        <Text c="dimmed" size="sm" style={{ marginBottom: 28, lineHeight: 1.6 }}>
          This action cannot be undone. Deleting this process will permanently remove all associated data, task, and configurations.
        </Text>
