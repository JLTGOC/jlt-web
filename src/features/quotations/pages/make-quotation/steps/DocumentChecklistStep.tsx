  ...
  return (
    ...
    >
      <Group gap="xs" justify="center">
        ...
        <Text size="sm">Drag and drop files here or click to upload</Text>
      </Group>
    </Dropzone>
  );
}
...
export function DocumentChecklistStep() {
  ...
  const { data } = useMakeQuotationEnums({
    service: state.clientInfo?.services,
  });
    ...
    <Stack gap="md">
      <SimpleGrid cols={{ base: 1, md: 2 }} spacing="lg">
        <Paper withBorder radius="md" p="lg">
          <Stack gap="md">
            <div>
              <Text fw={800}>DOCUMENTS CHECKLIST</Text>
              <Text size="sm" c="dimmed">
                Upload the required documents below. You can add other
                supporting files if needed.
              </Text>
            </div>
            {checklist.length === 0 && (
              <Text size="sm" c="dimmed">
                No checklist documents are required for this service.
              </Text>
            )}
            {checklist.map((item) => {
                  ...
                  <Stack gap="xs">
                    ...
                    {file ? (
                      <FilePill
                        ...
                        onRemove={() => clearChecklistFile(item)}
                      />
                    ) : (
                      <UploadDropzone
                        disabled={!checked}
        ...
        <Paper withBorder radius="md" p="lg">
          <Stack gap="md">
            <div>
              <Text fw={800}>OTHER FILES (SUPPORTING DOCUMENTS)</Text>
              <Text size="sm" c="dimmed">
                Upload any additional files that may support your quotation.
              </Text>
            </div>
            <Dropzone
              onDrop={(files) =>
                setOtherFiles((current) => [...current, ...files])
              }
              accept={ACCEPTED_MIME_TYPES}
              multiple
              p="lg"
      ...
      <Group justify="flex-end">
        <AppButton
          variant="primary"
          icon={RequestQuote}
          loading={state.isCreatingQuotation}
          disabled={state.isCreatingQuotation}
          onClick={() =>
            actions.submitDocuments({ checklistFiles, otherFiles })
          }
        >
          MAKE QUOTATION
        </AppButton>
